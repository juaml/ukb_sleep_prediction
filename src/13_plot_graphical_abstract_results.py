...
sns.set_context("paper", font_scale=3, rc={"xtick.labelsize": 24})
...
for t_score, t_df in best_models_data.items():
    if t_score in ["balanced_accuracy"]:
        this_scores = t_df[t_df["split"] == "test"]
        this_scores = this_scores[this_scores["features"] != "age and sex"]
        t_ax = axes
        # sns.swarmplot(
        #     x="target",
        #     y=t_score,
...
fig.suptitle(
    f"Comparison of models trained on brain features vs confounds"
)
fig.savefig(f"./figs/fig_graphical_abstract.pdf", bbox_inches="tight")
