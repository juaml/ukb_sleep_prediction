...
shared_data_dir = Path("/data/group/riseml/joblib_htcondor_shared_pca")
...
if target == "prs":
    ...
    creator.add("zscore")
    creator.add("pca", apply_to="*", n_components=20)
    search_params = None
    if model_name in ["rf", "et"]:
        ...
        predict_proba = "proba"
        n_jobs = -1
        # each job needs 0.6Gb. 26 CV * 0.6 Gb = 15.6 Gb
        ht_condor_recursion_level = 0
        # 2-level each job needs 0.6Gb. 26 CV * 12 * 5 * 0.6 Gb = 936 Gb
        throttle = [26, 60]
        search_params = {"kind": "grid", "scoring": "balanced_accuracy"}
        ...
        for t_ftype in feature_types:
            ...
            t_model.add("zscore")
            t_model.add("pca", n_components=20)
            t_model.add(
                LinearSVCHeuristicC(),
                name="linearsvcheuristicc",
                dual=False,
...
out_dir = Path(__file__).parent / "results_pca" / target
