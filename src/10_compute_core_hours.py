...
features = ["reduced_confounds", "full_confounds", "brain"]
...
for features_set in features:
    print(f"Processing feature set: {features_set}")
    for t_target in targets:
        t_log_path = log_path / t_target
        for t_model in models:
            if (
                t_model == "stacked_linearsvcheursiticc"
                and features_set != "brain"
            ):
                continue
            suffix = ""
            if features_set == "reduced_confounds":
                suffix = "_reduced_"
            elif features_set == "full_confounds":
                suffix = "_full_"
            log_file = (
                t_log_path / t_model / f"predict_{t_target}_{t_model}{suffix}.out"
            )
            ...
            if not log_file.exists():
                ...
                if start_time is None:
                    print(f"Start time not found in {log_file}")
                    continue
                if end_time is None:
                    print(
    ...
    if features_set == "brain":
    ...
    elif features_set == "reduced_confounds":
        batch_ids = {
            "gssvm_rbf": {
        ...
        batch_ids = {
            "gssvm_rbf": {
                "morning_evening_person_extreme": "jht-321f301c98af11f0a2d07cc2551e9c80-l0",
                "nap_day_extreme": "jht-323d7e4698af11f087f908c0eb24a372-l0",
                "doze_day": "jht-31fa7e8e98af11f0b98708c0eb24a8ca-l0",
            },
        }
        shared_joblib_dir = Path(
            # "/data/project/ukb_rls/joblib_htcondor_shared_confounds/"
            "/data/group/riseml/joblib_htcondor_shared_confounds"
        )
    for t_model, t_batchs in batch_ids.items():
        for t_target, t_id in t_batchs.items():
            ...
            print(f"Batch time: {t_batch_time} seconds")
            core_hours_data["target"].append(t_target)
            core_hours_data["model"].append(t_model)
            core_hours_data["features"].append(features_set)
...
core_hours_df["years"] = core_hours_df["hours"] / 24 / 365
