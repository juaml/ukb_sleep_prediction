...
logger = logging.getLogger('NIMRLS')
...
def log_versions(tbox_path=None):
    ...
    _safe_log(versions, 'argparse')
    _safe_log(versions, 'ipython')
    _safe_log(versions, 'datalad')
    _safe_log(versions, 'nest-asyncio')
    _safe_log(versions, 'sqlalchemy')
    _safe_log(versions, 'nimrls')
